...
vsi_nn_graph_t * vnn_CreateFaceBlendshapes
    (
...
/*-----------------------------------------
 ...
 -----------------------------------------*/
    ...
    /* @serving_default_input_points/0_0:out0 */
    memset( &attr, 0, sizeof( attr ) );
    attr.size[0] = 2;
    attr.size[1] = 146;
    attr.size[2] = 1;
    ...
    /* @StatefulPartitionedCall/0_153:out0 */
    memset( &attr, 0, sizeof( attr ) );
    attr.size[0] = 52;
    attr.dim_num = 1;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_NONE;
...
error:
    ...
    vnn_ReleaseFaceBlendshapes( graph, release_ctx );

    return NULL;
} /* vsi_nn_CreateFaceBlendshapes() */

void vnn_ReleaseFaceBlendshapes
    (
    vsi_nn_graph_t * graph,
    vsi_bool release_ctx
    )
